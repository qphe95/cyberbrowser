...
|---|-----------------|---------------|----------------|-----------------|
| 14 | `window.getComputedStyle()` | ✅ Merges inline `element.style` properties with the per-element computed-style table; adds `getPropertyPriority`, `item`, and `setProperty`; pseudo-element arg accepted but ignored. | Layout/scripts that read computed metrics for stamping or positioning get values that now include inline styles and computed properties. | `cyberbrowser/src/browser/console_api.cpp` |
| 15 | `CSSStyleSheet` / `adoptedStyleSheets` | ✅ Real `CSSStyleSheet` constructor with `type`, `href`, `ownerNode`, `ownerRule`, `title`, `disabled`, `cssRules`/`rules`, `insertRule`, `deleteRule`, `addRule`, `removeRule`, `replace` (returns resolved promise), and `replaceSync` (splits top-level rules). `document.styleSheets` and `ShadowRoot.styleSheets` getters added; `adoptedStyleSheets` setter already present on `ShadowRoot`. | Polymer's adopted-style-sheet fast path no longer throws. | `cyberbrowser/src/browser/css_api.cpp`; `cyberbrowser/src/browser_api_impl.cpp` |
| 16 | `document.implementation.createHTMLDocument()` | ✅ Returns a real `Document` node with `createElement`, `createTextNode`, `createComment`, `createDocumentFragment`, `createRange`, `createTreeWalker`, `importNode`, `querySelector(All)`, `getElementById`, `getElementsByTagName/ClassName`, and a `<html><head><title></title></head><body></body></html>` tree. | Used for safe style parsing/diffing in mutation observers (line 10025). | `cyberbrowser/src/browser_api_impl.cpp` |
| 17 | `Element.prototype.outerHTML` getter/setter | ✅ Getter serializes the element and its children; setter parses the replacement HTML, inserts the parsed nodes before the element, and removes the original element. | Polymer and sanitizers can now read/write `outerHTML`. | `cyberbrowser/src/browser/dom_api.cpp` |
| 18 | `Document.prototype.createComment()` | ✅ Returns a real Comment node (`nodeType` 8, `nodeName` `#comment`, `data`/`textContent`/`length`). | DOM scanners that expect comment nodes now receive them. | `cyberbrowser/src/browser_api_impl.cpp` |
| 19 | `requestIdleCallback()` / `requestAnimationFrame()` | ✅ `requestIdleCallback` still fires via a 0-delay timer, but the `IdleDeadline` now has `didTimeout=false` and `timeRemaining()` returns `50.0` ms instead of boolean `true`. `requestAnimationFrame` continues to fire as a 0-delay timer with a timestamp argument. | Polymer/ShadyDOM idle callbacks can now read a numeric remaining budget. | `cyberbrowser/src/browser/timer_api.cpp` |
