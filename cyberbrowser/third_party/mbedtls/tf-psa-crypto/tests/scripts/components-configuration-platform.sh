...
component_tf_psa_crypto_test_platform_calloc_macro () {
    msg "build: MBEDTLS_PLATFORM_{CALLOC/FREE}_MACRO enabled (Debug build)"
    ...
    msg "test: MBEDTLS_PLATFORM_{CALLOC/FREE}_MACRO enabled (Debug build)"
    make test
}
...
component_tf_psa_crypto_test_have_int32_cmake_new_bignum () {
    msg "build: gcc, force 32-bit bignum limbs, new bignum interface, test hooks (Debug build)"
    scripts/config.py unset MBEDTLS_HAVE_ASM
    scripts/config.py unset MBEDTLS_AESNI_C
    scripts/config.py unset MBEDTLS_AESCE_C
    ...
    cd $OUT_OF_SOURCE_DIR
    cmake -DCMAKE_C_COMPILER=gcc -DCMAKE_C_FLAGS="-DMBEDTLS_HAVE_INT32" -DCMAKE_BUILD_TYPE:String=Debug "$TF_PSA_CRYPTO_ROOT_DIR"
    make

    msg "test: gcc, force 32-bit bignum limbs, new bignum interface, test hooks (Debug build)"
    make test
}
